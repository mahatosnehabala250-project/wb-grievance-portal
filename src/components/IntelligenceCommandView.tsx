...
export function IntelligenceCommandView({ room }: { room?: string } = {}) {
  ...
  const runAction = useCallback(async (it: OpItem) => {
    ...
    if (it.execute.needs === 'officer') {
      ...
      if (!oid) { toast.error('Select an officer first'); return; }
      ...
      if (!window.confirm('This ticket will be marked RESOLVED and the citizen notified (WB-03). Confirm?')) return;
  ...
  const copyVillageBrief = useCallback((v: WapasVillage) => {
    ...
    navigator.clipboard.writeText(lines.join('\n'))
      .then(() => toast.success(`${v.village} brief copied — paste into WhatsApp`))
  ...
  const makeVillagePr = useCallback((v: WapasVillage) => {
    ...
    downloadVillagePrCard({ orgName: 'JanSunwai WB', village: v.village, count: v.count, avgRating: v.avgRating, categories, accent: '#BA7517' })
      .catch(() => toast.error('Could not build the PR card'));
  }, []);
        ...
        <div className="p-4 space-y-4">
          ...
          <Card className="border shadow-sm bg-gradient-to-br from-indigo-500/5 to-violet-500/5 border-indigo-500/20">
            <CardHeader className="pb-1 pt-3 px-4">
              <CardTitle className="text-xs font-semibold flex items-center gap-1.5 text-muted-foreground uppercase tracking-wider">
                ...
                <span className="text-[9px] normal-case font-normal">(ask anything about your area — Bengali, Hindi or English)</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="px-4 pb-3 space-y-2.5">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  placeholder="e.g. where should I visit this week?"
              ...
              {!advAnswer && !advLoading && (
                <div className="flex flex-wrap gap-1.5">
                  {[
                    'Where should I visit this week, and why?',
                    'Which officer is furthest behind?',
                    'What are the three most urgent items?',
                    'How does my area compare with peers?',
                  ].map((q) => (
              ...
              {advError && (
                <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 p-2.5 text-[11px] text-amber-700 dark:text-amber-400">
                  {advError === 'Advisor not configured — set DEEPSEEK_API_KEY'
                    ? 'AI Chief-of-Staff is off. An admin needs to set DEEPSEEK_API_KEY in Vercel.'
                    : advError}
                </div>
              )}
            ...
            <CardHeader className="pb-1 pt-3 px-4">
              <CardTitle className="text-xs font-semibold flex items-center justify-between text-foreground tracking-tight">
                <span className="flex flex-col gap-0.5">
                  ...
                  <span className="text-[10px] font-normal text-muted-foreground">a fused profile per area: grievance + anger + scheme failure + political, priority-ranked</span>
                </span>
                {!fusion && (
              ...
              <CardContent className="px-4 pb-3 space-y-2.5">
                {fusion.nodes.length === 0 ? (
                  ...
                  const reasonOf = (nd: FusionNode) => {
                    ...
                    return `${nd.grievance.active} active complaints + anger ${anger}`;
                  };
                  ...
                  <>
                    ...
                    <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl p-3"
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Where to focus today</div>
                          <div className="flex items-center gap-2">
                          ...
                          <Button size="sm" className="h-7 mt-2 text-[11px] px-2.5"
                            ...
                            <MapPin className="w-3.5 h-3.5 mr-1" /> Visit this area
                          </Button>
                        </div>
                    ...
                    {/* Trust footer */}
                    <details className="text-[10px] text-muted-foreground">
                      <summary className="cursor-pointer font-medium flex items-center gap-1"><Lock className="w-3 h-3" /> ⓘ What this score means</summary>
                      <div className="mt-1 pl-1"><div className="mb-1">Aggregate only · no individual profiling · DPDP/ECI-safe.</div><ul className="list-disc pl-4 space-y-0.5">{fusion.caveats.map((c, i) => <li key={i}>{c}</li>)}</ul></div>
                    </details>
                  </>
                  );
                })()}
            ...
            <CardHeader className="pb-1 pt-3 px-4">
              <CardTitle className="text-xs font-semibold flex items-center justify-between text-muted-foreground uppercase tracking-wider">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] normal-case font-normal">(chain of command, and where complaints stall)</span>
                </span>
                {!network && (
                ...
                {network.weakestLinks.length > 0 && (
                  <div>
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-red-500 mb-1 flex items-center gap-1">
                      <ShieldAlert className="w-3 h-3" /> Weakest links — where the backlog is stuck in the chain
                    </div>
                    <div className="space-y-1">
            ...
            <CardHeader className="pb-1 pt-3 px-4">
              <CardTitle className="text-xs font-semibold flex items-center justify-between text-muted-foreground uppercase tracking-wider">
                <span className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] normal-case font-normal">(what to do today — approve in one tap)</span>
                </span>
                {!operations && (
              ...
              <CardContent className="px-4 pb-3 space-y-3">
                ...
                {!operations.canWrite && (
                  <div className="text-[10px] rounded-lg bg-muted/30 p-2 text-muted-foreground">
                    Read-only role — yeh queue <b>advisory</b> (no approve button).
                  </div>
                )}
                ...
                <div className="rounded-lg bg-muted/30 p-2">
                  <div className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Not yet (honestly)</div>
                ...
                <details className="text-[10px] text-muted-foreground">
                  <summary className="cursor-pointer font-semibold">⚠ How this queue works ({operations.caveats.length})</summary>
            ...
            <CardHeader className="pb-1 pt-3 px-4">
              <CardTitle className="text-xs font-semibold flex items-center justify-between text-foreground tracking-tight">
                <span className="flex flex-col gap-0.5">
                  ...
                  <span className="text-[10px] font-normal text-muted-foreground">from the complaint text: root cause, anger, recurring names</span>
                </span>
                {!nlp ? (
                  ...
                  <>
                    ...
                    {nlp.clusters.length > 0 && (
                      <div>
                        <div className="text-[11px] font-bold flex items-center gap-1 mb-1.5 text-fuchsia-600 dark:text-fuchsia-400">
                          <Layers className="w-3.5 h-3.5" /> One action, many complaints resolved
                        </div>
                        <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {nlp.angerHotspots.length > 0 && (
                        <div>
                          <div className="text-[11px] font-bold flex items-center gap-1 mb-1.5 text-red-500"><Flame className="w-3.5 h-3.5" /> Where anger is highest</div>
                          <div className="space-y-1">
          ...
          <Card className={`border shadow-sm border-violet-500/20 ${show('field') ? '' : 'hidden'}`}>
            ...
            {wapas && (
              <CardContent className="px-4 pb-3 space-y-1.5">
                {wapas.length === 0 ? (
                  <div className="py-4 text-center text-xs text-muted-foreground">No resolved complaints yet — resolve some, then come back 💪</div>
                ) : wapas.map(v => {
