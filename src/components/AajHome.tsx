...
import { RefreshCw, Flag, MessageSquare, ArrowRight, Zap, Share2 } from 'lucide-react';
...
export function AajHome({ user, branding, onOpenActions, onOpenIntelligence }: {
  ...
  const copyBrief = () => {
    ...
    const lines = [
      ...
      ...(top3.length ? ['', 'Aaj ke kaam:', ...top3.map((it, i) => `${i + 1}. ${it.title}`)] : []),
      ...(chiefLine ? ['', `🧭 ${chiefLine}`] : []),
      '',
  ...
  return (
    <div className="space-y-5" style={{ '--cc-accent': branding.accent } as React.CSSProperties}>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">{brief?.scope.label || branding.orgName}{branding.leaderName ? ` · ${branding.leaderName}` : ''}</div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          {brief && (
            <Button variant="outline" size="sm" className="h-7 px-2 text-[11px]" onClick={copyBrief}>
              <Share2 className="w-3.5 h-3.5 mr-1" /> Brief
            </Button>
          )}
      ...
      {loading && !brief ? (
        <div className="text-sm text-muted-foreground py-10 text-center">Aaj ka briefing taiyaar ho raha hai…</div>
      ) : !brief ? (
        ...
        <>
          ...
          <div>
            <div className="text-[13px] font-semibold mb-2 flex items-center gap-1.5">
              <Flag className="w-3.5 h-3.5" style={{ color: branding.accent }} /> Aaj ke 3 kaam
