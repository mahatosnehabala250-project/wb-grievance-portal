 ...
 * numbers that matter, AI-picked "today's three actions" (top of the Level-10 action
...
export function AajHome({ user, branding, onOpenActions, onOpenIntelligence }: {
  ...
  const chiefLine = (() => {
    ...
    if (hs) parts.push(`heaviest pressure on ${hs.name}`);
  ...
  const copyBrief = () => {
    ...
    navigator.clipboard.writeText(lines.join('\n'))
      .then(() => toast.success('Brief copied — paste it into WhatsApp or Telegram'))
  ...
  const downloadImage = () => {
    ...
    }).catch(() => toast.error('Could not generate the image'));
  };
